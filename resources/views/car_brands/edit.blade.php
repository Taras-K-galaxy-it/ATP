...
@section('content')
    ...
    <div class="flex justify-center items-center h-screen">
        <form action="{{ route('brands.update', $brand->id) }}" class="w-96 p-6 rounded drop-shadow-lg bg-gray-50" method="POST">
            @csrf
            @method('PUT')

            <label for="car_brands">Update Brand</label>
            <input type="text" class="mt-1 p-0.5 block w-full rounded-md border border-black drop-shadow-lg" name="car_brands" value="{{ $brand->car_brands }}">
